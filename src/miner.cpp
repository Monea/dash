...
// ***TODO*** ScanHash is not yet used in Monea
//
...
{
    LogPrintf("MoneaMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("monea-miner");
    ...
    try {
        ...
        while (true) {
            ...
            LogPrintf("MoneaMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("MoneaMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("MoneaMiner -- runtime error: %s\n", e.what());
        return;
    }
}
