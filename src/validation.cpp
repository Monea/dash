...
#if defined(NDEBUG)
# error "Monea Core cannot be compiled without assertions."
#endif
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    int step = 60000;
    int startIncrease = 10000; // payments start at block..
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // MNA : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(MNA): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(MNA): couldn't find masternode or superblock payments"),
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    //pindexNew->nChainWork = (pindexNew->pprev ? pindexNew->pprev->nChainWork : 0) + GetBlockProof(*pindexNew); // DASH
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // MNA : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(MNA): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(MNA): spork is off, skipping transaction locking checks\n");
    }
...
*/
    ...
    // Check Previous Block
        ...
        if (block.GetAlgo() == ALGO_NEOSCRYPT)     { LogPrintf("Algo is Neoscypt \n");}
        if (block.GetAlgo() == ALGO_ARGON2D) { LogPrintf("Algo is Argon2d \n");}
        if (block.GetAlgo() == ALGO_YESCRYPT)     { LogPrintf("Algo is Yescrypt \n");}
    }
    int nHeight = pindex->nHeight;
...
bool static LoadBlockIndexDB()
{
    ...
    {
        CBlockIndex* pindex = item.second;
        //pindex->nChainWork = (pindex->pprev ? pindex->pprev->nChainWork : 0) + GetBlockProof(*pindex); // DASH
